...
$hours        = get_field( 'fw3_opening_time', 'option' );
$contact      = get_field( 'fw3_contact', 'option' );
$address      = get_field( 'fw3_address', 'option' );
$btn          = get_field( 'fw3_button', 'option' );

$social_icons = get_field( 'fw3_social_icons', 'options' );
...
<div class="f-widget col-span-2">
	...
	<ul class="list grid grid-cols-1 md:grid-cols-2 gap-x-10 2xl:gap-x-14 gap-y-8">
		...
		<li>
			<?php
			if ( $social_icons && $social_icons['fw3_social_heading'] ) {
				echo '<h4 class="font-bold">' . esc_html( $social_icons['fw3_social_heading'] ) . '</h4>';
			}
			?>
			<!-- <h4 class="font-bold">Social</h4> -->
			<ul class="flex space-x-6">
				<?php
				if ( $social_links['twitter'] && in_array( 'Twitter', $social_icons['fw3_social_icons'], true ) ) {
					echo '<li><a class="hover:text-theme-color" href="' . esc_url( $social_links['twitter'] ) . '" target="_blank"><i class="fa fa-twitter"></i></a></li>';
				}
				if ( $social_links['linkedin'] && in_array( 'LinkedIn', $social_icons['fw3_social_icons'], true ) ) {
					echo '<li><a class="hover:text-theme-color" href="' . esc_url( $social_links['linkedin'] ) . '" target="_blank"><i class="fa fa-linkedin"></i></a></li>';
				}
				if ( $social_links['instagram'] && in_array( 'Instagram', $social_icons['fw3_social_icons'], true ) ) {
					echo '<li><a class="hover:text-theme-color" href="' . esc_url( $social_links['instagram'] ) . '" target="_blank"><i class="fa fa-instagram"></i></a></li>';
				}
				if ( $social_links['facebook'] && in_array( 'Facebook', $social_icons['fw3_social_icons'], true ) ) {
